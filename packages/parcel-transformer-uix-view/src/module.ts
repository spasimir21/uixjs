...
async function compileViewModule(viewModule: ViewModuleData) {
  // prettier-ignore
  ...
  for (const stylesheet of viewModule.stylesheets) compiledStylesheets.push(await compileStylesheet(stylesheet));
