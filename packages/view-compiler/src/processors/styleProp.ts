...
function processStyleProps(element: Node, view: ViewData) {
  ...
  for (const attribName of attribsToBeProcessed) {
    view.instructions.push(
      `u.styleProperty(${viewSelector}, '${attribName.slice(1)}', () => (${element.getAttribute(attribName)}))`
    );
