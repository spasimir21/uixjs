...
export default class HTMLElement extends Node {
  ...
  public set innerHTML(content: string) {
    ...
    resetParent(this.childNodes, null as any);
  ...
  public get rawAttributes() {
    ...
    if (this.rawAttrs) {
      const re = /([a-zA-Z0-9-_:()[\]#!*_.@]+)(?:\s*=\s*((?:'[^']*')|(?:"[^"]*")|\S+))?/g;
...
export function base_parse(data: string, options = {} as Partial<Options>) {
  ...
  const root = new HTMLElement(null as any, {}, '', null, [0, data.length], voidTag, options);
